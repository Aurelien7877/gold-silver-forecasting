...
| Chronos-2 with covariates | Target plus past-only Gold/Silver-related covariates | Not yet run | This is the strongest pending external experiment. It needs a successful local checkpoint download and must reuse the same dates, horizon, costs and tests. |
...
The next high-value experiment is not another unreported architecture name. It is a Chronos-2 covariate benchmark with an authenticated local checkpoint, followed by a pre-registered comparison against the current Gold ExtraTrees and Silver directional-logistic winners. If Chronos-2 does not beat them under the same protocol, the local models remain the stronger practical choice for this snapshot.
