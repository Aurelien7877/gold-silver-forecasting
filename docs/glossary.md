...
- **XReg / covariate regression** — TimesFM’s XReg mode fits a small in-context linear model on covariates and combines it with the pretrained forecast. The covariate arrays must include the forecast horizon, so this project supplies only lagged signals whose next value is known from date `t`.
