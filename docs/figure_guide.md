...
- **`model_benchmark.png`** — Bars compare net locked-test Sharpe across the evaluated families, including the shared global iTransformer probe, for Gold and Silver. Gold ExtraTrees reaches `1.187` and Silver directional logistic `1.315`, but the bars are a locked-period audit rather than permission to choose retrospectively.
...
- **White Reality Check p-value** — The candidate-aware block bootstrap repeatedly selects the maximum Sharpe among the available candidates under a no-skill null. The current p-values (`0.090` Gold, `0.021` Silver) are useful evidence, not a universal SOTA certificate.
